...
function New-ClaudeConfig {
    ...
    foreach ($lang in $SelectedLanguages) {
        # Core language rules (unconditional, always apply)
...
"@
            ($frontmatter + $fileContent + "`n") | Out-File -FilePath $outputFile -Encoding UTF8 -NoNewline
            $relativePath = $outputFile.Replace($Script:ProjectRoot, "").TrimStart("\", "/")
            Write-SuccessMessage "Created $relativePath"
        }
...
"@
                ($frontmatter + $docContent + "`n") | Out-File -FilePath $outputFile -Encoding UTF8 -NoNewline
                $relativePath = $outputFile.Replace($Script:ProjectRoot, "").TrimStart("\", "/")
                Write-SuccessMessage "Created $relativePath"
            }
        }
